...
-- Ditambahkan: Rotasi Lokasi Otomatis, Auto-Sell Cerdas & Deteksi Inventaris Penuh
...
-- [[ BARU ]] Remote untuk Auto-Sell
local sellAllRemote = ResolveRemote("RF/SellAllItems")
local updateAutoSellThresholdRemote = ResolveRemote("RF/UpdateAutoSellThreshold")
...
-- [[ BARU ]] Sistem Rotasi Lokasi
local LocationRotation = {
    enabled = false,
    sessionId = 0,
    rotationInterval = 5, -- (menit) Cek setiap 5 menit
    minTimeAtLocation = 2, -- (menit) Waktu minimum di satu lokasi sebelum pindah
    minEfficiencyThreshold = 5, -- (persen) Pindah jika rate ikan langka < 5%
    minFishSample = 10, -- Jumlah ikan minimum untuk data valid
    lastRotationCheck = 0,
    timeSpentAtCurrentLocation = 0
}

-- [[ BARU ]] Sistem Manajemen Inventaris
local InventoryManager = {
    enabled = false,
    isFull = false,
    lastSellTime = 0,
    autoSellThreshold = "RARE", -- Jual semua di bawah Rare secara default
    shopLocation = CFrame.new(-650.971191, 208.693695, 711.10907) -- Default ke Kohana
}
...
local Dashboard = {
    ...
    sessionStats = {
        ...
        locationStartTime = tick() -- [[ BARU ]] Waktu mulai di lokasi saat ini
    },
    heatmap = {},
    optimalTimes = {}
}
...
local function LogFishCatch(fishName, location)
    ...
    if rarity ~= "COMMON" and rarity ~= "UNCOMMON" then -- [[ DIUBAH ]] Hanya hitung Rare ke atas
...
local function LocationTracker()
    while true do
        ...
        if newLocation ~= Dashboard.sessionStats.currentLocation then
            ...
            Dashboard.sessionStats.locationStartTime = tick() -- [[ BARU ]] Reset timer saat pindah lokasi
...
local function secureInvoke(remote, ...)
    ...
    if not ok then
        -- [[ BARU ]] Deteksi inventaris penuh dari pesan error
        if res and type(res) == "string" and res:lower():find("inventory is full") then
            InventoryManager.isFull = true
            Notify("Inventory", "⚠️ Inventaris terdeteksi penuh!")
        end
...
local function DoSmartCycle()
    ...
    if rodRemote and rodRemote:IsA("RemoteFunction") then 
        local ok, res = pcall(function() rodRemote:InvokeServer(timestamp) end)
        if not ok and res and type(res) == "string" and res:lower():find("inventory") then
            InventoryManager.isFull = true
        end
    end
...
local function DoSecureCycle()
    ...
    if rodRemote then
        ...
        if not ok then 
            ...
            if res and type(res) == "string" and res:lower():find("inventory") then
                InventoryManager.isFull = true
            end
        end
    end
...
local function DoFastCycle()
    ...
    if rodRemote then
        local ok, res = pcall(function() rodRemote:InvokeServer(workspace:GetServerTimeNow()) end)
        if not ok then 
...
-- [[ BARU ]] Fungsi untuk menangani inventaris penuh
local function HandleFullInventory()
    Notify("Inventory Manager", "📦 Inventaris penuh! Memulai proses penjualan...")
    
    local lastLocation = LocalPlayer.Character.HumanoidRootPart.CFrame
    
    -- Teleport ke toko
    Notify("Inventory Manager", "Teleport ke Toko...")
    LocalPlayer.Character.HumanoidRootPart.CFrame = InventoryManager.shopLocation
    task.wait(3)
    
    -- Jual item
    Notify("Inventory Manager", "Menjual item...")
    if sellAllRemote then
        local ok, res = pcall(function() sellAllRemote:InvokeServer() end)
        if ok then
            Notify("Inventory Manager", "✅ Item berhasil dijual!")
        else
            Notify("Inventory Manager", "❌ Gagal menjual item.")
        end
    end
    task.wait(2)
    
    -- Kembali ke lokasi terakhir
    Notify("Inventory Manager", "Kembali ke lokasi memancing...")
    LocalPlayer.Character.HumanoidRootPart.CFrame = lastLocation
    task.wait(3)
    
    InventoryManager.isFull = false
    Notify("Inventory Manager", "Siklus selesai, melanjutkan memancing.")
end
...
local function AutofishRunner(mySession)
    ...
    Dashboard.sessionStats.locationStartTime = tick() -- [[ BARU ]]
    ...
    while Config.enabled and sessionId == mySession do
        -- [[ BARU ]] Cek inventaris sebelum setiap siklus
        if InventoryManager.enabled and InventoryManager.isFull then
            HandleFullInventory()
        end
...
-- [[ BARU ]] Fungsi untuk menjalankan rotasi lokasi
local function LocationRotationRunner(mySessionId)
    Notify("Location Rotation", "🗺️ Sistem Rotasi Lokasi Aktif.")
    while LocationRotation.enabled and LocationRotation.sessionId == mySessionId do
        -- Tunggu sesuai interval
        task.wait(LocationRotation.rotationInterval * 60)
        
        if not Config.enabled then continue end -- Hanya berjalan jika autofish aktif

        -- Cek waktu minimum di lokasi
        local timeInLocation = (tick() - Dashboard.sessionStats.locationStartTime) / 60
        if timeInLocation < LocationRotation.minTimeAtLocation then
            Notify("Location Rotation", string.format("Belum pindah, baru %.1f menit di lokasi ini.", timeInLocation))
            continue
        end

        local currentLocation = Dashboard.sessionStats.currentLocation
        local currentStats = Dashboard.locationStats[currentLocation]

        -- Cek apakah data cukup
        if not currentStats or currentStats.total < LocationRotation.minFishSample then
            Notify("Location Rotation", "Data di lokasi saat ini belum cukup untuk evaluasi.")
            continue
        end

        local currentEfficiency = (currentStats.rare / currentStats.total) * 100
        Notify("Location Rotation", string.format("Efisiensi saat ini: %.1f%%", currentEfficiency))

        -- Cek apakah efisiensi di bawah ambang batas
        if currentEfficiency < LocationRotation.minEfficiencyThreshold then
            Notify("Location Rotation", "Efisiensi rendah! Mencari lokasi yang lebih baik...")
            
            local bestLocationName = nil
            local bestLocationEfficiency = -1

            -- Cari lokasi terbaik dari data yang ada
            for locName, stats in pairs(Dashboard.locationStats) do
                if locName ~= currentLocation and stats.total >= LocationRotation.minFishSample then
                    local efficiency = (stats.rare / stats.total) * 100
                    if efficiency > bestLocationEfficiency then
                        bestLocationEfficiency = efficiency
                        bestLocationName = locName
                    end
                end
            end

            -- Jika ditemukan lokasi yang lebih baik
            if bestLocationName and bestLocationEfficiency > currentEfficiency then
                Notify("Location Rotation", string.format("Pindah ke %s (Efisiensi: %.1f%%)", bestLocationName, bestLocationEfficiency))
                
                -- Cari CFrame dari nama lokasi
                local targetCFrame = nil
                for islandName, cframe in pairs(islandLocations) do
                    if islandName:find(bestLocationName) then
                        targetCFrame = cframe
                        break
                    end
                end
                ...
                if targetCFrame then
                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        Dashboard.sessionStats.currentLocation = bestLocationName
                        Dashboard.sessionStats.locationStartTime = tick()
                        task.wait(2) -- Beri waktu untuk teleport
                    end
                else
                    Notify("Location Rotation", "Gagal menemukan koordinat untuk " .. bestLocationName)
                end
            else
                Notify("Location Rotation", "Tidak ada lokasi lain yang lebih baik saat ini.")
            end
        end
    end
    Notify("Location Rotation", "🗺️ Sistem Rotasi Lokasi Berhenti.")
end
...
local function BuildUI()
    ...
    -- [[ BARU ]] Location Rotation Section
    local rotationSection = Instance.new("Frame", fishingAIScrollFrame)
    rotationSection.Size = UDim2.new(1, -10, 0, 90)
    rotationSection.Position = UDim2.new(0, 5, 0, 335)
    rotationSection.BackgroundColor3 = Color3.fromRGB(45,45,52)
    rotationSection.BorderSizePixel = 0
    Instance.new("UICorner", rotationSection)

    local rotationTitle = Instance.new("TextLabel", rotationSection)
    rotationTitle.Size = UDim2.new(1, -20, 0, 20)
    rotationTitle.Position = UDim2.new(0, 10, 0, 5)
    rotationTitle.Text = "🗺️ Auto Location Rotation"
    rotationTitle.Font = Enum.Font.GothamBold
    rotationTitle.TextSize = 14
    rotationTitle.TextColor3 = Color3.fromRGB(150,150,255)
    rotationTitle.BackgroundTransparency = 1
    rotationTitle.TextXAlignment = Enum.TextXAlignment.Left

    local rotationLabel = Instance.new("TextLabel", rotationSection)
    rotationLabel.Size = UDim2.new(0.65, -10, 0, 25)
    rotationLabel.Position = UDim2.new(0, 15, 0, 30)
    rotationLabel.Text = "Rotasi Lokasi: Disabled"
    rotationLabel.Font = Enum.Font.GothamSemibold
    ...
    rotationLabel.TextYAlignment = Enum.TextYAlignment.Center

    local rotationToggle = Instance.new("TextButton", rotationSection)
    rotationToggle.Size = UDim2.new(0, 70, 0, 24)
    rotationToggle.Position = UDim2.new(1, -80, 0, 31)
    rotationToggle.Text = "🔴 OFF"
    rotationToggle.Font = Enum.Font.GothamBold
    rotationToggle.TextSize = 11
    ...
    local rotationStatus = Instance.new("TextLabel", rotationSection)
    rotationStatus.Size = UDim2.new(1, -20, 0, 25)
    rotationStatus.Position = UDim2.new(0, 10, 0, 60)
    rotationStatus.Text = string.format("Pindah jika efisiensi < %d%% setelah %d menit.", LocationRotation.minEfficiencyThreshold, LocationRotation.minTimeAtLocation)
    rotationStatus.Font = Enum.Font.Gotham
    rotationStatus.TextSize = 11
    rotationStatus.TextColor3 = Color3.fromRGB(180,180,180)
    rotationStatus.BackgroundTransparency = 1
    rotationStatus.TextXAlignment = Enum.TextXAlignment.Left
    
    -- [[ BARU ]] Inventory Manager Section
    local inventorySection = Instance.new("Frame", fishingAIScrollFrame)
    inventorySection.Size = UDim2.new(1, -10, 0, 120)
    inventorySection.Position = UDim2.new(0, 5, 0, 435)
    inventorySection.BackgroundColor3 = Color3.fromRGB(45,45,52)
    inventorySection.BorderSizePixel = 0
    Instance.new("UICorner", inventorySection)

    local inventoryTitle = Instance.new("TextLabel", inventorySection)
    inventoryTitle.Size = UDim2.new(1, -20, 0, 20)
    inventoryTitle.Position = UDim2.new(0, 10, 0, 5)
    inventoryTitle.Text = "📦 Manajemen Inventaris"
    inventoryTitle.Font = Enum.Font.GothamBold
    inventoryTitle.TextSize = 14
    inventoryTitle.TextColor3 = Color3.fromRGB(255, 180, 100)
    inventoryTitle.BackgroundTransparency = 1
    inventoryTitle.TextXAlignment = Enum.TextXAlignment.Left

    local invManagerLabel = Instance.new("TextLabel", inventorySection)
    invManagerLabel.Size = UDim2.new(0.65, -10, 0, 25)
    invManagerLabel.Position = UDim2.new(0, 15, 0, 30)
    invManagerLabel.Text = "Auto Sell/Teleport: Disabled"
    invManagerLabel.Font = Enum.Font.GothamSemibold
    invManagerLabel.TextSize = 12
    invManagerLabel.TextColor3 = Color3.fromRGB(200,200,200)
    invManagerLabel.BackgroundTransparency = 1
    invManagerLabel.TextXAlignment = Enum.TextXAlignment.Left
    invManagerLabel.TextYAlignment = Enum.TextYAlignment.Center

    local invManagerToggle = Instance.new("TextButton", inventorySection)
    invManagerToggle.Size = UDim2.new(0, 70, 0, 24)
    invManagerToggle.Position = UDim2.new(1, -80, 0, 31)
    invManagerToggle.Text = "🔴 OFF"
    invManagerToggle.Font = Enum.Font.GothamBold
    invManagerToggle.TextSize = 11
    invManagerToggle.BackgroundColor3 = Color3.fromRGB(160,60,60)
    invManagerToggle.TextColor3 = Color3.fromRGB(255,255,255)
    Instance.new("UICorner", invManagerToggle)
    
    local autoSellLabel = Instance.new("TextLabel", inventorySection)
    autoSellLabel.Size = UDim2.new(0.4, 0, 0, 25)
    autoSellLabel.Position = UDim2.new(0, 15, 0, 60)
    autoSellLabel.Text = "Jual di bawah:"
    autoSellLabel.Font = Enum.Font.Gotham
    autoSellLabel.TextSize = 12
    ...
    local autoSellDropdown = Instance.new("TextButton", inventorySection)
    autoSellDropdown.Size = UDim2.new(0.6, -20, 0, 24)
    autoSellDropdown.Position = UDim2.new(0.4, 10, 0, 60)
    autoSellDropdown.Text = InventoryManager.autoSellThreshold .. " ▼"
    autoSellDropdown.Font = Enum.Font.GothamSemibold
    autoSellDropdown.TextSize = 11
    autoSellDropdown.BackgroundColor3 = Color3.fromRGB(60,60,66)
    autoSellDropdown.TextColor3 = Color3.fromRGB(255,255,255)
    Instance.new("UICorner", autoSellDropdown)
    
    local inventoryStatus = Instance.new("TextLabel", inventorySection)
    inventoryStatus.Size = UDim2.new(1, -20, 0, 25)
    inventoryStatus.Position = UDim2.new(0, 10, 0, 90)
    inventoryStatus.Text = "Status Inventaris: OK"
    inventoryStatus.Font = Enum.Font.Gotham
    inventoryStatus.TextSize = 11
    inventoryStatus.TextColor3 = Color3.fromRGB(100,255,100)
    inventoryStatus.BackgroundTransparency = 1
    inventoryStatus.TextXAlignment = Enum.TextXAlignment.Left

    -- Set canvas size for fishing AI scroll
    fishingAIScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 565)
    ...
    rotationToggle.MouseButton1Click:Connect(function()
        ...
        if LocationRotation.enabled then
            rotationToggle.Text = "🟢 ON"
            rotationToggle.BackgroundColor3 = Color3.fromRGB(70,170,90)
            rotationLabel.Text = "Rotasi Lokasi: Enabled"
            rotationLabel.TextColor3 = Color3.fromRGB(100,255,150)
            
            LocationRotation.sessionId = LocationRotation.sessionId + 1
            task.spawn(function() LocationRotationRunner(LocationRotation.sessionId) end)
        else
            rotationToggle.Text = "🔴 OFF"
            rotationToggle.BackgroundColor3 = Color3.fromRGB(160,60,60)
            rotationLabel.Text = "Rotasi Lokasi: Disabled"
    ...
    -- [[ BARU ]] Inventory Manager Toggle
    invManagerToggle.MouseButton1Click:Connect(function()
        InventoryManager.enabled = not InventoryManager.enabled
        if InventoryManager.enabled then
            invManagerToggle.Text = "🟢 ON"
            invManagerToggle.BackgroundColor3 = Color3.fromRGB(70,170,90)
            invManagerLabel.Text = "Auto Sell/Teleport: Enabled"
            invManagerLabel.TextColor3 = Color3.fromRGB(100,255,150)
            Notify("Inventory Manager", "📦 Sistem Manajemen Inventaris Aktif.")
        else
            invManagerToggle.Text = "🔴 OFF"
            invManagerToggle.BackgroundColor3 = Color3.fromRGB(160,60,60)
            invManagerLabel.Text = "Auto Sell/Teleport: Disabled"
            invManagerLabel.TextColor3 = Color3.fromRGB(200,200,200)
            Notify("Inventory Manager", "📦 Sistem Manajemen Inventaris Nonaktif.")
        end
    end)
    
    -- [[ BARU ]] Auto Sell Dropdown
    local rarityLevels = {"COMMON", "UNCOMMON", "RARE", "EPIC", "LEGENDARY", "MYTHIC"}
    local currentRarityIndex = 3 -- Default to RARE
    autoSellDropdown.MouseButton1Click:Connect(function()
        currentRarityIndex = currentRarityIndex + 1
        if currentRarityIndex > #rarityLevels then
            currentRarityIndex = 1
        end
        InventoryManager.autoSellThreshold = rarityLevels[currentRarityIndex]
        autoSellDropdown.Text = InventoryManager.autoSellThreshold .. " ▼"
        
        if updateAutoSellThresholdRemote then
            pcall(function()
                updateAutoSellThresholdRemote:InvokeServer(InventoryManager.autoSellThreshold)
                Notify("Auto-Sell", "Ambang batas auto-sell diatur ke: " .. InventoryManager.autoSellThreshold)
            end)
        else
            Notify("Auto-Sell", "Remote untuk update auto-sell tidak ditemukan.")
        end
    end)
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        LocationRotation.enabled = false; LocationRotation.sessionId = LocationRotation.sessionId + 1 -- [[ BARU ]]
        InventoryManager.enabled = false -- [[ BARU ]]
    ...
    local function DashboardUpdater()
        while true do
            ...
            -- [[ BARU ]] Update status inventaris di UI
            if InventoryManager.isFull then
                inventoryStatus.Text = "Status Inventaris: PENUH!"
                inventoryStatus.TextColor3 = Color3.fromRGB(255,100,100)
            else
                inventoryStatus.Text = "Status Inventaris: OK"
                inventoryStatus.TextColor3 = Color3.fromRGB(100,255,100)
            end
