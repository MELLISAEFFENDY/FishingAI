...
local function generateRandomJumpTime()
    -- Random time between 1.7-10 minutes (102-600 seconds) for better security
    return math.random(102, 600)
end

-- Enhanced jump function with multiple methods (proven successful)
local function performAntiAfkJump()
    -- Comprehensive character validation
    if not LocalPlayer.Character then
        print("⚠️ AntiAFK: No character found")
        return false
    end
    
    local humanoid = LocalPlayer.Character:FindFirstChild("Humanoid")
    if not humanoid then
        print("⚠️ AntiAFK: No humanoid found")
        return false
    end
    
    local rootPart = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not rootPart then
        print("⚠️ AntiAFK: No root part found")
        return false
    end
    
    -- Check if character can jump (avoid jumping while already jumping/falling)
    ...
    if state == Enum.HumanoidStateType.Freefall or 
       ...
       state == Enum.HumanoidStateType.Jumping then
        print("⚠️ AntiAFK: Character not grounded, skipping jump")
        -- Still update timing to prevent spam
        local currentTime = tick()
    ...
    print("🦘 AntiAFK: Executing jump...")
    
    -- Multiple jump methods for maximum compatibility (proven in test)
    -- Method 1: Standard jump
    humanoid.Jump = true
    
    -- Method 2: Force jump state with small delay
    task.spawn(function()
        task.wait(0.1)
        humanoid.Jump = true
        
        -- Method 3: Alternative using ChangeState (backup method)
        task.wait(0.1)
        pcall(function()
    ...
    -- Calculate and display next jump time
    ...
    print(string.format("✅ AntiAFK: Jump executed! Next jump in %dm %ds", nextJumpMinutes, nextJumpSeconds))
    Notify("AntiAFK", string.format("✅ Jump performed! Next in %dm %ds", nextJumpMinutes, nextJumpSeconds))
...
local function AntiAfkRunner(mySessionId)
    ...
    print("🚀 AntiAFK: System started with enhanced jump detection")
    Notify("AntiAFK", "🚀 Enhanced AntiAFK system started")
    ...
    while AntiAFK.enabled and AntiAFK.sessionId == mySessionId do
        ...
        if currentTime >= AntiAFK.nextJumpTime then
            local success = performAntiAfkJump()
            if not success then
                -- If jump failed, retry in 30 seconds
                print("⚠️ AntiAFK: Jump failed, retrying in 30 seconds")
                AntiAFK.nextJumpTime = currentTime + 30
            end
        end
        
        -- Optional: Show countdown for next jump (only every 2 minutes to avoid spam)
        local timeLeft = AntiAFK.nextJumpTime - currentTime
        if timeLeft > 0 and timeLeft <= 600 and math.floor(timeLeft) % 120 == 0 then
            local minutes = math.floor(timeLeft / 60)
            local seconds = math.floor(timeLeft % 60)
            print(string.format("⏰ AntiAFK: Next jump in %dm %ds", minutes, seconds))
        end
        
        task.wait(1) -- Check every second
    end
    
    print("🛑 AntiAFK: System stopped")
    Notify("AntiAFK", "🛑 Enhanced AntiAFK system stopped")
end
