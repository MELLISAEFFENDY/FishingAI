...
-- Tombol urutan remote seperti old.lua
local comboBtn = Instance.new("TextButton")
comboBtn.Size = UDim2.new(0, 140, 0, 30)
comboBtn.Position = UDim2.new(0, 10, 0, 70)
comboBtn.Text = "🔗 Combo: Equip→Play→Charge"
comboBtn.Font = Enum.Font.GothamSemibold
comboBtn.TextSize = 10
comboBtn.BackgroundColor3 = Color3.fromRGB(70, 120, 200)
comboBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
comboBtn.Parent = quickSection
...
executeBtn.MouseButton1Click:Connect(function()
    ...
    -- Jika remote ChargeFishingRod dan argumen kosong, isi otomatis
    if remoteName:lower():find("chargefishingrod") and (args == nil or args == "") then
        local serverTime = nil
        pcall(function()
            serverTime = workspace:GetServerTimeNow()
        end)
        if serverTime then
            args = tostring(serverTime)
            argsInput.Text = tostring(serverTime)
        else
            args = "1"
            argsInput.Text = "1"
        end
...
comboBtn.MouseButton1Click:Connect(function()
    ...
    remoteInput.Text = "EquipRod"
    argsInput.Text = "1"
    executeRemote("EquipRod", "1")
    updateHistory()
    task.wait(0.2)
    ...
    if serverTime then
        ...
        argsInput.Text = tostring(serverTime)
        executeRemote("ChargeFishingRod", tostring(serverTime))
    else
        print("[Combo] ChargeFishingRod:InvokeServer(1)")
        remoteInput.Text = "ChargeFishingRod"
        argsInput.Text = "1"
...
floatingBtn.MouseButton1Click:Connect(function()
    ...
    if isUIVisible then
        ...
        print("🔧 Remote Tester UI shown")
        Notify("Remote Tester", "🔧 UI opened!")
    else
        floatingBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 150)
        print("🔧 Remote Tester UI hidden")
        Notify("Remote Tester", "🔧 UI hidden!")
    end
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.Return or input.KeyCode == Enum.KeyCode.KeypadEnter then
        if remoteInput:IsFocused() or argsInput:IsFocused() then
            executeRemote(remoteInput.Text, argsInput.Text)
            updateHistory()
        end
...
print("🎮 Controls: F5=Scan, Enter=Execute, Click floating button=Toggle UI")
